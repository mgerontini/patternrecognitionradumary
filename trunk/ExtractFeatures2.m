...
for i = 1 : size(differenced_data,2)
    ...
    if (data(3,i) ~= 0) % only calculate a direction when the pen is down, else set a direction of -500 (the angle is between -180 and 180, so -300 is not a valid angle)
        features(1,i) = atan2(differenced_data(2,i),differenced_data(1,i)) + 0.01*randn(1,1); % add some noise to avoid 0 variance
    else
        features(1,i) = -500 + randn(1,1); % add some noise so as not to have 0 variance
    end
end
    ...
    for i = 1 : numberOfStrokes
        for j = stroke_begin(i) : stroke_end(i)
            ...
            if (i ~= 1) % not the first stroke
                features(3:4,j) = data(1:2,stroke_begin(i))-data(1:2,stroke_end(i-1))+0.001*randn(2,1); % this is the x-offset between where the current stroke begins and where the previous one ended
            else
                features(3:4,j) = 0.001*randn(2,1); % for the first stroke there is no offset, so we just put in a little 0-mean noise, instead of having 0 everywhere.
            end
        end
    end
end
